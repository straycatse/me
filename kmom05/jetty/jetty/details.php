...
if ($jettyPosition) {
    ...
    $sql = "SELECT * FROM Jetty WHERE id = ?";
    $stmt = $db->prepare($sql);
...
<h2>Position: <?php echo $jettyPosition?></h2>
<h2>Type: <?php echo $boatType?></h2>
<h2>Engine: <?php echo $boatEngine?></h2>
<h2>Length: <?php echo $boatLength?></h2>
<h2>Width: <?php echo $boatWidth?></h2>
<h2>Owner: <?php echo $ownerName?></h2>
...
$rows = null;
foreach ($res as $row) {
    $jettyPosition = htmlentities($row['id']);
    $rows .= "<tr>";
    $rows .= "<td><a href='?page=details?jettyPosition=$jettyPosition'>$jettyPosition</a></td>";
